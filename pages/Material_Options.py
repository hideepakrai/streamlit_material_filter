...
with st.sidebar:
    st.header("Filters")
    search = st.text_input("Search material_id (LIKE)")  # matches integer id as string
...
params = {}
if search:
    where.append("CAST(mo.material_id AS CHAR) LIKE :q")
    params["q"] = f"%{search}%"
...
st.dataframe(
    ...
    column_config={
        ...
        "material_option_ids": st.column_config.TextColumn("Material Option IDs"),
    },
